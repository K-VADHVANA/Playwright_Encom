...
```
Error: page.isVisible: Test ended.
Call log:
    - checking visibility of locator('//enviso-profileimage')

    at CloudDashboardPage.Logout (C:\Users\nipatel\Playwright\playwright-pom\pages\CloudDashboardPage.ts:30:37)
```
...
```ts
  ...
  29 |
> 30 |         console.log(await this.page.isVisible(this.Elements.profileMnu));
     |                                     ^ Error: page.isVisible: Test ended.
  31 |         await this.page.click(this.Elements.profileMnu);
  32 |
  33 |         await this.page.waitForTimeout(5000);
  34 |         await this.page.click(this.Elements.logoutLnk);
  35 |     }
  36 | }
```