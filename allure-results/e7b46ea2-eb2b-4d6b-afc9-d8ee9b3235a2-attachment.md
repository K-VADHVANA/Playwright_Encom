...
Call log:
  ...
  - retrying click action
    ...
    - scrolling into view if needed
    - done scrolling
    - <div class="toasts" _ngcontent-ng-c2429199863="">…</div> from <enviso-alert _nghost-ng-c2429199863="" _ngcontent-ng-c2985647552="">…</enviso-alert> subtree intercepts pointer events
  - retrying click action
    - waiting 100ms
    ...
    at MyOffersPage.createOfferWithName (C:\Users\kvadhvana\playwright-pom\pages\Myoffers.ts:53:35)
...
```ts
  ...
  22 |
  23 |     this.Elements = {
  24 |       sidebarExpandBtn: this.page.locator('.icons8-double-right'),
  25 |       myOffersMenuItem: this.page.getByText('My offers'),
  26 |       createOfferBtn: this.page.locator('#offer-list-btn-create-offer-id'),
  27 |       ticketCard: this.page.locator('#offer-activate-only-id'),
  28 |       offerNameInput: this.page.locator('#offer-create-text-offer-name-id'),
  29 |       offerImageInput: this.page.locator('input[type="file"]'), // Hidden input for file upload
  30 |       activateBtn: this.page.getByRole('button', { name: 'Activate' }),
  31 |       errorMessage: this.page.getByText('Offer name is required'),
  32 |       donebtn: this.page.getByRole('button', { name: 'Done' }),
  33 |     };
  34 |   }
  35 |
  36 |   async goToMyOffers() {
  37 |     await this.Elements.sidebarExpandBtn.click();
  38 |     await this.Elements.myOffersMenuItem.click();
  39 |     await this.Elements.sidebarExpandBtn.click(); // Collapse again
  40 |   }
  41 | async createOfferWithName(name: string) {
  42 |   await this.Elements.createOfferBtn.click();
  43 |   await this.Elements.ticketCard.click();
  44 |   await this.Elements.offerNameInput.fill(name);
  45 |
  46 |   // ✅ Fixed relative path
  47 |   
  48 |   const imagePath = path.resolve(__dirname, '../utils/offerImage.png');
  49 |   await this.Elements.offerImageInput.setInputFiles(imagePath);
  50 |   if (!fs.existsSync(imagePath)) {
  51 |   throw new Error(`Image file not found at: ${imagePath}`);
  52 | }
> 53 |   await this.Elements.donebtn.click();
     |                                   ^ TimeoutError: locator.click: Timeout 10000ms exceeded.
  54 | }
  55 | }
  56 |
